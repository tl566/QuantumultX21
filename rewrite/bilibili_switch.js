...
async function SwitchRegion(title, url, body) {
	const Group = $.read('BiliArea_Policy') || '📺 DomesticMedia' || 'BiliBili'; //Your blibli policy group name.
	const CN = $.read('BiliArea_CN') || 'DIRECT'; //Your China sub-policy name.
	const TW = $.read('BiliArea_TW') || '🇹🇼 sub-policy' || '台湾节点' || '台湾'; //Your Taiwan sub-policy name.
	const HK = $.read('BiliArea_HK') || '🇭🇰 sub-policy' || '香港节点' || '香港'; //Your HongKong sub-policy name.
	const DF = $.read('BiliArea_DF') || '🏁 sub-policy' || '台湾节点' || '台湾'; //Sub-policy name used after region is blocked(e.g. url 404)
